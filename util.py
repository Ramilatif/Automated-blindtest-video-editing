...
def createClip(path: str, timeCode_S: int,timeCode_E: int):
    ...
    txt_clip = txt_clip.set_position((75, 900)).set_duration(final_clip.duration)
    background = background.set_position((75, 900)).set_duration(final_clip.duration)
...
def generate_countdown_video(n):
    ...
    for i in range(n, -1, -1):
        txt = TextClip(str(i), fontsize=70, color='white', size=(1440, 1080))
...
def createBlindClip(path: str, timeCode_S: int,timeCode_E: int):
    ...
    blindClip = blindClip.set_audio(audio)
...
def createFinalClip (path: str, timeCode_S: int,timeCode_E: int):
    blindClip = createBlindClip(path, timeCode_S, timeCode_S+ 15)
